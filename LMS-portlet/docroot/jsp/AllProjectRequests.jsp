...
List<LMSLeaveInformation>allLeaveRequest=new ArrayList<LMSLeaveInformation>();
 ...
 Date currentDate = df.parse(Date);
    ...
    Date inputStartDate = df.parse(inputDate);
     ...
     if(inputStartDate.before(currentDate)||check !=0 && check != 1)//past dated autoapproval leaves
        ...
        // System.out.println("past dated check"+approverleave.getStartDate());
         allLeaveRequest.add(approverleave);
         
         // System.out.println("past dated leaves added to list"+allLeaveRequest.size());
...
<table id="employeeLeavehistory" class="hover">
...
<portlet:param name="Sno" value="<%=String.valueOf(leaveinfo.getPrimaryKey())%>"/>
        ...
        <tr>
           <td><%= LMSEmployeeDetailsLocalServiceUtil.getLMSEmployeeDetails(leaveinfo.getEmployeeId()).getEmployeeName()%></td>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
     ...
     <aui:form method="post"  action="<%=approveLeaveURL%>" >
      ...
      <aui:input type="textarea" resizable="false" name="approveComments" label="" maxlength="200" placeholder="Enter only 200 characters"/>  
...
<%} %>
    ...
    %>
    <%for(LMSLeaveInformation approvedLeave : allLeaveRequest){ %>
     <tr>
